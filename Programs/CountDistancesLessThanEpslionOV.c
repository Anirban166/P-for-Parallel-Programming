...
void printDataset(struct point *data);
...
{	
  ...
  // Sort the point data based on either x/y values:
  qsort(pointData, N, sizeof(struct point), compareFunctionX);  
  // qsort(data, N, sizeof(pointData), compareFunctionY);  
  // printDataset(data);    
  ...
  double distance, epsilonSquare = epsilon * epsilon, tstartCPU = omp_get_wtime();
  int i, j;  
  ...
  for(i = 0; i < N - 1; ++i)
  {
    for(j = i + 1; j < N; ++j)
    {   
      distance = pow((pointData[i].x - pointData[j].x), 2) + pow((pointData[i].y - pointData[j].y), 2);
  ...
  printf("\nTotal number of distances between points that are within a search radius (epsilon) of %.1f, as computed by the CPU: %llu", epsilon, (2 * countDistancesLessThanEpsilon) + N);
...
int compareFunctionX(const void *a, const void *b) 
{
  const struct point *pointA = (struct point *) a;
  const struct point *pointB = (struct point *) b;
